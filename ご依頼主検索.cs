...
namespace is2AdminClient
{
	...
	public class ご依頼主検索 : 共通フォーム
	{
		...
		private void InitializeComponent()
		{
			...
			this.tex依頼主カナ = new is2AdminClient.共通テキストボックス();
			...
			this.tex依頼主カナ.ImeMode = System.Windows.Forms.ImeMode.Hiragana;
			this.tex依頼主カナ.Location = new System.Drawing.Point(118, 28);
			this.tex依頼主カナ.MaxLength = 10;
			this.tex依頼主カナ.Name = "tex依頼主カナ";
			this.tex依頼主カナ.Size = new System.Drawing.Size(174, 23);
			this.tex依頼主カナ.TabIndex = 1;
			this.tex依頼主カナ.Text = "";
			// 
			// labご依頼主名
			// 
...
// MOD 2007.02.02 東都）高木 [会員名]→[お客様名] START
//			axGT依頼主.set_RowsText(0, "|お客様コード|会員名|部門名|ご依頼主コード|ご依頼主名|住所|部門ＣＤ|");
			axGT依頼主.set_RowsText(0, "|お客様コード|お客様名|部門名|ご依頼主コード|ご依頼主名|住所|部門ＣＤ|");
// MOD 2007.02.02 東都）高木 [会員名]→[お客様名] END
...
//			if(!全角チェック(tex依頼主カナ,"ご依頼主名")) return;
// DEL 2007.01.18 東都）高木 画面レイアウト変更 START
			...
			try
			{
				...
				sKey[2] = tex依頼主カナ.Text;
				sKey[3] = gs店所コード.Trim();
				texメッセージ.Text = "検索中．．．";
				if(sv_maintenance == null) sv_maintenance = new is2maintenance.Service1();
				s依頼主一覧 = sv_maintenance.Get_Goirainusi(gsaユーザ,sKey);
